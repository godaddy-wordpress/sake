...
module.exports = (config, options) => {
  ...
  // set up translations sources
  exports.buildTranslationsConfig = function () {
    let projectID = (config.translations && config.translations.project) ? config.translations.project : (config.plugin && config.plugin.id) ? config.plugin.id : path.basename(process.cwd())
    config.translations = {
      // this is where the public instance of GlotPress is located, defaults to https://translations.skyverge.com/glotpress/
      glotpress: (config.translations && config.translations.glotpress) ? config.translations.glotpress : 'https://translations.skyverge.com/glotpress/',
      // this is the project identifier within GlotPress, defaults to plugin ID
      project: projectID,
      // this is the project translations source, defaults to <project>.pot
      source: (config.translations && config.translations.source) ? config.translations.source : projectID + '.pot'
    }
  }
  ...
  exports.buildPaths = function () {
    ...
    config.paths.assetPaths.javascriptSources = [
      ...
      `!${config.paths.assetPaths.js}/blocks/**/*.js`,
      `!${config.paths.assetPaths.js}/blocks/**/*.min.js`
    ]

    config.paths.assetPaths.blockSources = [
      `${config.paths.assetPaths.js}/blocks/src/**/*.js`,
      `!${config.paths.assetPaths.js}/blocks/src/**/*.min.js`,
      `!${config.paths.assetPaths.js}/blocks/**/*.min.js`
    ]
  ...
  exports.buildFrameworkConfig = function () {
    if (config.framework) {
      ...
      let frameworkInstalledViaComposer = config.composer && dottie.get(config.composer, 'require.skyverge/wc-plugin-framework')

      // try to determine the framework path based on composer config
      if (frameworkInstalledViaComposer && !dottie.exists(config, 'paths.framework.base')) {
  ...
  exports.buildDeployConfig = function () {
    ...
    if (config.deploy.type) {
      // `production` represents the production repo for this plugin, which can be either a github or WP SVN repo. We'll try to determine the repo from the following values (in order):
    ...
    // - DEPLOY_DOCS env variable
    // - deploy.docs value from sake.config.js
    // - default sales docs repo slug
    let docsInput = process.env.DEPLOY_DOCS || dottie.get(config, 'deploy.docs') || 'wc-plugins-sales-docs'
    config.deploy.docs = parseRepoConfig(docsInput, 'dev')
  ...
  exports.getMainPluginFile = function (fullpath) {
    ...
    try {
      ...
      log.warn(chalk.yellow('Error when trying to find plugin main file: ', e.message))
    }
    ...
    if (fileName) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (changelog.plugin_version === '' && lines[i].indexOf('version') !== -1) {
          ...
          changelog.date = lines[i].substr(0, lines[i].indexOf(' - '))
        }
  ...
  /**
   * Extracts plugin's WooCommerce.com product ID from plugin headers.
   */
  exports.getPluginWooId = function () {
    let id = ''

    if (config.plugin.mainFile) {
      let contents = fs.readFileSync(`${config.paths.src}/${config.plugin.mainFile}`, 'utf8')

      // example header: Woo: 18660:157b35835ab64abe650e5fb17af4c631
      let match = contents.match(/Woo: ([0-9]+):[a-zA-Z0-9]+/)
  ...
  exports.getChangelogErrors = function () {
    ...
    let date = config.plugin.changelogDate

    if (!date || !/[0-9]+\.nn\.nn/.test(date)) {
      errors.push(`Changelog date ${date} is not a valid date for deploy`)
    }
  ...
  /**
   * Checks whether the given ENV variables have been set.
   *
   * @throws Error
   */
  exports.validateEnvironmentVariables = (variables) => {
    ...
    if (errors.length) {
      exports.throwError('Environment variables missing or invalid: \n * ' + errors.join('\n * '))
    }
  }
  ...
  /**
   * Throws an error with setTimout.
   *
   * Makes throwing errors from within Promise catch blocks possible,
