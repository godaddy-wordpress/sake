...
module.exports = (config, options) => {
  ...
  exports.buildDeployConfig = function () {
    ...
    let parseRepoConfig = (input, env, deployType) => {
      ...
      if (input.indexOf('github') > -1) {
        ...
        repo.user = process.env.WP_SVN_USER
        ...
        // we seem to be dealing with just the github repo slug, we'll assume woocommerce is the owner if the environment is production and throw an error for the dev environment
        if (env === 'production') {
          repo.owner = 'woocommerce'
        } else {
          exports.throwError(`Could not determine the owner of the GitHub repo with slug '${input}'.\n\nPlease use a configuration option (deploy.dev or deploy.docs) or an environment variable (DEPLOY_DEV or DEPLOY_DOCS) to define the username and slug for the repo.`)
        }
    ...
    let docsInput = process.env.DEPLOY_DOCS || dottie.get(config, 'deploy.docs')
  ...
  /**
   * Checks if the current CLI invocation includes the build task or deploy tasks.
   * Used to conditionally skip sourcemap generation during builds and deployments.
   *
   * @return boolean
   */
  exports.isBuildTask = () => {
    ...
    return args.includes('build') || 
           ...
           args.includes('copy_to_wc_repo') || 
    return ['build', 'deploy', 'copy_to_wc_repo', 'copy_to_wp_repo'].some(task => args.includes(task))
  }
