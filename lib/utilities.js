...
module.exports = (config, options) => {
  ...
  exports.buildDeployConfig = function () {
    ...
    if (config.deploy.type === 'wc') {
      // parse owner and repo from repo url
    ...
    if (config.deploy.type === 'wp') {
      config.deploy = _.merge({
        user: process.env.WP_SVN_USER || 'SkyVerge',
        repo: 'http://plugins.svn.wordpress.org/' + config.plugin.id
      }, config.deploy)

      if (!config.paths.wpAssets) {
  ...
  // TODO: evaluate if we need this anymore (currently the same stuff is handled by config.deploy, but perhaps options are more appropriate)
  ...
  exports.getVersionBump = function () {
    ...
    let bump = config.deploy.version

    if (bump === 'custom') {
      bump = config.deploy.version_custom
    }
  ...
  exports.getWPRepoPath = function () {
    return path.join(config.paths.tmp, config.plugin.id)
  }

  exports.getWCRepoPath = function () {
    return this.resolvePath(process.env.WT_REPOS_PATH + '/' + config.deploy.github.production.repo + '/')
  }
