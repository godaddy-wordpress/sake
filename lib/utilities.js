...
module.exports = (config, options) => {
  ...
  exports.buildPluginConfig = function () {
    ...
    config.plugin = {
      name: config.plugin.name || changelog.plugin_name,
      id: config.plugin.id || path.basename(process.cwd()),
  ...
  exports.buildDeployConfig = function () {
    ...
    config.deploys = _.merge({
      github: {
        internal: {
          owner: process.env.INTERNAL_GITHUB_OWNER || 'skyverge',
          repo: process.env.INTERNAL_GITHUB_REPO || config.plugin.id
        }
      }
    }, config.deploys)
    ...
    if (config.deployType === 'wc') {
      config.deploys.github = _.merge({
        external: {
          owner: process.env.EXTERNAL_GITHUB_OWNER || 'woocommerce',
          repo: process.env.EXTERNAL_GITHUB_REPO || config.plugin.id
        }
