...
module.exports = (gulp, plugins, sake) => {
  ...
  gulp.task('copy:build', () => {
    ...
    return gulp.src(paths, { base: sake.config.paths.src, allowEmpty: true, encoding: false })
      .pipe(filter)
      .pipe(plugins.replace(/\/\*# sourceMappingURL=.*?\*\/$/mg, '')) // remove CSS-style source mapping references
      .pipe(plugins.replace(/\/\/# sourceMappingURL=.*$/mg, '')) // remove JS-style source mapping references
